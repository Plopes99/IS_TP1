...
class CSVtoXMLConverter:
    ...
    def salvar_xml_em_arquivo(self, nome_arquivo):
        xml_tree = self.to_xml()
        xml_str = ET.tostring(xml_tree, encoding='utf-8', method='xml').decode()

        # Criar um documento DOM a partir da string XML
        dom = md.parseString(xml_str)

        # Abrir o arquivo em modo de escrita
        with open(nome_arquivo, 'w', encoding='utf-8') as arquivo:
            # Escrever o XML formatado no arquivo
            arquivo.write(dom.toprettyxml())
